...
def load_nfl_data():
    ...
    # Return values must match what's expected when unpacking
...
# Update sidebar with diagnostics
with st.sidebar:
    st.divider()
    ...
    if val_acc > 0:
        ...
        st.caption("Model training in progress...")

# Status messages
if not sched_db.empty and 'season' in sched_db.columns:
    sched_source = sched_db['season'].max()
    st.success(f"✅ Active Season: {sched_source}")
else:
    st.warning("Schedule data unavailable")

st.info(f"📅 Data Up To: {get_last_nfl_date()}")
if hfa_db:
    st.caption("HFA Data: 3 Years Loaded")
